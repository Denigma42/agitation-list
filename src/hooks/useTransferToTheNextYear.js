import useUpdateDistrict from "./useUpdateDistrict";
...
export default function useTransferToTheNextYear() {
    ...
    const transferToTheNextYear = async () => {
        try {
            ...
            const { data: platoons = [] } = await window.electronAPI.getAllDistricts();
            const { data: students = [] } = await window.electronAPI.getAllSchools();
            ...
            for (const platoon of sortedPlatoons) {
                // Пропускаем архивные районы при переводе вперед
                ...
                if (newCourse > typeLimit) {
                    ...
                    await updateDistrict(platoon.id, {
                        ...platoon,
                        isInArchive: true,
                ...
                const platoonStudents = students.filter(s => s.districtId === platoon.id);
